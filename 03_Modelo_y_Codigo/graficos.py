...
def plot_figura_05(res, stat):
    ...
    l3 = ax2.plot(years, yield_ar1, marker="s", color=C["aluar"], lw=2.2, linestyle="-.", markersize=5.5, zorder=3, label="Rendimiento (%)")
...
def plot_figura_11(res, stat):
    ...
    ax.set_xlabel("EV/EBITDA UDM (x)", fontsize=9.5, fontweight="bold")
...
def plot_figura_15(res, stat):
    ...
    fig, ax = create_fig(figsize=(4.8, 2.7), margins=dict(left=0.14, right=0.94, top=0.92, bottom=0.15))
    steps = ["PV FCFF", "+ PV TV", "= EV", "− Deuda", "= Equity", "Objetivo"]
...
def plot_figura_17(res, stat):
    ...
    ax2.set_ylabel("Precio Objetivo (ARS)", fontsize=8.8, fontweight="bold")
...
def plot_figura_22(res, stat):
    ...
    horizons = ["UDM", "Paso 1", "Paso 2", "Paso 3", "Mediana"]
...
def plot_figura_31(res, stat, muestra):
    ...
    ax.set_xlabel("Precio Objetivo Simulado Clayton (ARS)", fontsize=9.5, fontweight="bold")
