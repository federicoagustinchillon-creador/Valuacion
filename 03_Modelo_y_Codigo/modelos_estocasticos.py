...
# ===========================================================================
def m14_cir_embi(embi_pb, dt: float = None, embi_ar1_fit: tuple = None, theta_target: float = None) -> dict:
    """
    Calibracion de un proceso de Cox-Ingersoll-Ross (CIR) sobre la serie
    del EMBI+ Argentina (en puntos basicos), con verificacion de la
    condicion de Feller y comparacion formal (AIC) contra el AR(1) lineal.

    Discretizacion (Euler-Maruyama):
        r_(t+1) = r_t + kappa*(theta - r_t)*dt + sigma*sqrt(r_t)*sqrt(dt)*eps_t

    Si theta_target se especifica (ej. 441 pb del DCF), se ancla la media de
    equilibrio de largo plazo a dicho objetivo macroeconomico de convergencia.
    """
    r = np.asarray(embi_pb, dtype=float)
    if dt is None:
        dt = 1.0 / 252.0 if len(r) > 100 else 1.0
    r0, r1 = r[:-1], r[1:]
    n = len(r0)
    ...
    theta = float(theta_target) if theta_target is not None else theta_ar1
    ...
    ll_ar1 = float(-0.5 * n * (np.log(2 * np.pi * sigma2_ar1) + 1))
    k_params_ar1 = 3  # phi, theta, sigma
    aic_ar1 = 2 * k_params_ar1 - 2 * ll_ar1

    frecuencia = "diaria (dt=1/252)" if dt < 0.01 else "anual (dt=1.0)"
    ...
    return {
        ...
        "feller_ratio": float(feller / (sigma ** 2)),
        ...
        "dt": float(dt),
        "nota": (f"Estimacion sobre {n} transiciones en frecuencia {frecuencia}. "
                 "Verifica formalmente la condicion de Feller y superioridad por AIC."),
    }
...
def s0_ahorro_energia_peal_v(mw: float, factor_planta: float, ahorro_usd_mwh: float,
                              wacc: float) -> float:
    """
    Valor presente (USD) del ahorro incremental de energia de la expansion
    PEAL V, modelado como una perpetuidad a la tasa WACC del caso base --
    ES la formula que ya documentaba el .tex en prosa (Seccion "Estructura
    del Modelo DCF y Valor de la Opcion Real PEAL V"), ahora como funcion:

        S0 = MW * factor_planta * 8.760 h/anio * ahorro_USD/MWh / WACC

    El WACC (no Rf) es la tasa correcta aca: S0 es el valor de un activo
    real (el ahorro operativo futuro), no el precio de la opcion sobre ese
    activo -- eso se descuenta aparte, a la tasa libre de riesgo, dentro de
    lsmc_opcion_americana() (ver m18_opcion_real_peal_v).
    """
    ahorro_anual_usd = mw * factor_planta * 8_760.0 * ahorro_usd_mwh
    return ahorro_anual_usd / wacc


def m18_opcion_real_peal_v(capex_usdmm: float, s0_usdmm: float, rf: float,
    ...
    Insumos y de donde sale cada uno:
      - capex_usdmm: el I0 de ejercicio de la opcion (inversion total
        estimada para completar la expansion de 312 MW), distinto del CAPEX
        ya ejecutado en 2025 (~USD 243,9 MM, ver m4_estados/graficos.py
        "Pico CAPEX PEAL V") -- este ultimo es gasto pasado, no el strike.
      - rf: tasa libre de riesgo (m1_mercado, ^TNX). Longstaff-Schwartz
        clasico pide valuar bajo la medida de riesgo neutral, es decir
        descontar a Rf, no al WACC -- el WACC se usa aparte para construir
        S0 (ver s0_ahorro_energia_peal_v), no para precificar la opcion.
      - sigma_lme: la volatilidad del proceso de precios del LME ya
        calibrado en graficos.py (figura_26, GBM sobre log-retornos
        mensuales del LME).
      - s0_usdmm: valor presente del ahorro incremental de energia,
        s0_ahorro_energia_peal_v(mw=312, factor_planta, ahorro_usd_mwh, wacc).
    """
    res = lsmc_opcion_americana(S0=s0_usdmm, K=capex_usdmm, r=rf, sigma=sigma_lme,
                                 T=horizonte_anios, tipo="call", **kwargs)
    return {
        **res,
        "supuestos": {
            "capex_usdmm_fuente": "I0 de ejercicio: inversion total estimada para completar la expansion de 312 MW",
            "sigma_lme_fuente": "Volatilidad del proceso GBM del LME calibrado en graficos.py (figura_26)",
            "rf_fuente": "m1_mercado (^TNX) -- tasa libre de riesgo, correcta para LSMC bajo medida neutral al riesgo",
            "s0_usdmm_fuente": "s0_ahorro_energia_peal_v(): perpetuidad del ahorro de energia a WACC",
        },
    }
