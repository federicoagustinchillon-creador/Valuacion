...
--------------------------------------------------------------
   1. Costo del capital propio: CAPM ajustado por riesgo pais (Dumrauf,
      ...
      ventas totales (Aluar exporta ~83,8% de su produccion (aluar.com.ar, seccion Mercados)).
...
LAMBDA_AR = 0.1617              # ventas al mercado interno / ventas totales: 62.871 Tn
                                 # mercado interno vs 326.017 Tn exportacion (aluar.com.ar,
                                 # seccion Mercados) = 83,83% exportado, 16,17% interno
...
# Opcion real PEAL V (expansion eolica de 312 MW) -- supuestos operativos sin
# fuente propia en el repositorio, documentados aca en vez de en prosa suelta:
PEAL_V_MW = 312.0
PEAL_V_FACTOR_PLANTA = 0.45            # punto base; rango de sensibilidad 40%-50%
PEAL_V_AHORRO_USD_MWH = 27.5           # punto medio del rango USD 25-30/MWh
PEAL_V_I0_USDMM = 400.0e6              # ~USD 400 MM, inversion de la Etapa V (Mejor Energia, ago-2024)
...
# ===========================================================================
# M2B — VALIDACION ECONOMETRICA ADICIONAL (ADF, Phillips-Perron, ARCH-LM,
# Quandt-Andrews, Sobol) — calculada de verdad sobre la serie real de
# retornos, no citada de memoria. Cierra la Tabla de Validacion Econometrica
# y la seccion de Sensibilidad de Sobol con numeros trazables a este archivo.
# ===========================================================================
def _sup_f_chow(y, X, k, lo, hi):
    """sup-F de Quandt-Andrews sobre la regresion y = X @ beta, recorte [lo,hi)."""
    def ssr(y_, X_):
        beta, *_ = np.linalg.lstsq(X_, y_, rcond=None)
        r = y_ - X_ @ beta
        return float(r @ r), beta

    ssr_r, beta_full = ssr(y, X)
    n = len(y)
    best_f, best_tau = -np.inf, None
    for tau in range(lo, hi):
        ssr1, _ = ssr(y[:tau], X[:tau])
        ssr2, _ = ssr(y[tau:], X[tau:])
        f_tau = ((ssr_r - (ssr1 + ssr2)) / k) / ((ssr1 + ssr2) / (n - 2 * k))
        if f_tau > best_f:
            best_f, best_tau = f_tau, tau
    return best_f, best_tau, beta_full


def m2b_validacion_econometrica(panel: pd.DataFrame, cc: dict, pro: dict, mkt: dict,
                                 deuda_neta_usdmm: float, est: dict, semilla: int = 42) -> dict:
    """
    Cinco pruebas de validacion que antes se citaban en el .tex sin ningun
    calculo real detras (ADF, Phillips-Perron, ARCH-LM, Quandt-Andrews y
    Sobol): se computan aca, sobre las mismas fuentes que usa el resto del
    motor (panel de precios, DCF de m7), para que cada numero de la Tabla de
    Validacion Econometrica y de la Sensibilidad de Sobol tenga un origen
    verificable en este archivo.
    """
    r = panel["alua_usd"].pct_change().dropna()
    r_mkt = panel["sp500_ret"].pct_change().dropna()

    adf_t, adf_p, adf_lags, *_ = adfuller(r, autolag="AIC")
    pp = PhillipsPerron(r.values)
    arch_lm, arch_lm_p, _, _ = het_arch(r.values, nlags=5)

    # Quandt-Andrews sobre la regresion de mercado (r_ALUA = a + b*r_SP500),
    # el mismo par que produce el Beta OLS de m1_mercado. Recorte 15%-85%
    # (estandar de Andrews 1993). El p-value se estima por bootstrap
    # (remuestreo de residuos bajo H0 de parametros constantes) porque
    # statsmodels no trae las criticas asintoticas de Andrews tabuladas.
    df_qa = pd.concat([r.rename("y"), r_mkt.rename("x")], axis=1, join="inner").dropna()
    y_qa, x_qa = df_qa["y"].values, df_qa["x"].values
    n_qa = len(y_qa)
    X_qa = np.column_stack([np.ones(n_qa), x_qa])
    lo, hi = int(0.15 * n_qa), int(0.85 * n_qa)
    f_obs, tau_obs, beta_full = _sup_f_chow(y_qa, X_qa, 2, lo, hi)

    rng = np.random.default_rng(semilla)
    resid_full = y_qa - X_qa @ beta_full
    n_boot = 400
    boot_f = np.empty(n_boot)
    for b in range(n_boot):
        y_boot = X_qa @ beta_full + rng.choice(resid_full, size=n_qa, replace=True)
        boot_f[b], _, _ = _sup_f_chow(y_boot, X_qa, 2, lo, hi)
    qa_p = float((boot_f >= f_obs).mean())
    fecha_quiebre = str(df_qa.index[tau_obs].date())
    beta_pre, *_ = np.linalg.lstsq(X_qa[:tau_obs], y_qa[:tau_obs], rcond=None)
    beta_post, *_ = np.linalg.lstsq(X_qa[tau_obs:], y_qa[tau_obs:], rcond=None)

    # --- Sobol sobre el precio objetivo del DCF -----------------------------
    # Mismo dcf_fn "de bolsillo" que m9_monte_carlo, pero escalar (Sobol
    # necesita evaluar f() en cada punto de la malla, no puede filtrar
    # corridas invalidas como hace el Monte Carlo vectorizado).
    wacc0, g0 = cc["wacc"], cc["g_perpetuidad"]
    fcff = [pro["proyecciones"][y]["fcff"] for y in ANIOS_PROY]
    nopat_terminal = pro["proyecciones"][ANIOS_PROY[-1]]["nopat"]
    acciones_mm, ccl = mkt["acciones_mm"], mkt["ccl"]

    def dcf_fn(wacc_shock, g_shock, margin_shock):
        w = wacc0 + wacc_shock
        gg = g0 + g_shock
        spread = max(w - gg, 0.005)   # piso continuo, analogo al filtro w-g>0.01 de m9
        factores = [(1 + w) ** i for i in range(len(fcff))]
        pv = sum(f * (1 + margin_shock) / d for f, d in zip(fcff, factores))
        tv = nopat_terminal * (1 + margin_shock) * (1 + gg) / spread
        pv_tv = tv / (1 + w) ** (len(fcff) - 1)
        eq = (pv + pv_tv) - deuda_neta_usdmm
        return max(eq / acciones_mm * ccl, 0.0)
    ...
    sd_shock_margen = cv_margen / np.sqrt(len(margenes))

    sobol = ME.m17_sobol_sensibilidad(dcf_fn, sd_wacc, sd_g, sd_shock_margen, semilla=semilla)

    return {
        "adf": {"t": float(adf_t), "p": float(adf_p), "lags": int(adf_lags), "n_obs": int(len(r))},
        "phillips_perron": {"z_t": float(pp.stat), "p": float(pp.pvalue), "lags": int(pp.lags)},
        "arch_lm_lag5": {"lm": float(arch_lm), "p": float(arch_lm_p)},
        "quandt_andrews": {
            "sup_f": float(f_obs), "p_bootstrap": qa_p, "n_boot": n_boot,
            "recorte": "15%-85%", "n_obs": n_qa, "fecha_quiebre": fecha_quiebre,
            "beta_pre": float(beta_pre[1]), "beta_post": float(beta_post[1]),
        },
        "sobol_precio_objetivo": sobol,
    }
...
def m6_costo_capital(mkt: dict, est: dict) -> dict:
    ...
    # exporta ~83,8% de su produccion y factura en dolares al precio LME.
...
def anexo(mkt, cc, est, panel) -> dict:
    ...
    for semilla_cf in range(5):
        ...
        r_sim = mu + sd * _z_cf(rng_cf.standard_normal(200_000))
        for cl, q in ((0.90, 0.10), (0.95, 0.05), (0.99, 0.01)):
...
def extensiones_estocasticas(mkt, cc, est, panel, cache_mercado) -> dict:
    ...
    Incluye M18 (opcion real PEAL V): s0_ahorro_energia_peal_v() construye el
    valor presente del ahorro de energia como perpetuidad a WACC y
    m18_opcion_real_peal_v() la precifica via LSMC a la tasa libre de riesgo
    (ver PEAL_V_* mas arriba para los supuestos operativos sin fuente propia
    en el repositorio: MW, factor de planta, ahorro USD/MWh, I0 de ejercicio).
    ...
    embi_serie = cache_mercado["embi"].dropna().values if "embi" in cache_mercado.columns else st["embi_hist"]["values"]
    m14 = ME.m14_cir_embi(embi_serie, theta_target=float(cc["embi"] * 10000.0))

    m15 = ME.m15_merton_jumps(cache_mercado)

    m16 = ME.m16_copula_colas(cache_mercado)

    # M18 -- Opcion real PEAL V (ver s0_ahorro_energia_peal_v / m18_opcion_real_peal_v)
    lme_mensual = (cache_mercado.set_index("Date")["lme"]
                   .resample("ME").last().interpolate(method="linear").bfill().ffill())
    log_ret_lme = np.diff(np.log(lme_mensual.values))
    sigma_lme_anual = float(np.std(log_ret_lme, ddof=1) * np.sqrt(12))

    s0_base = ME.s0_ahorro_energia_peal_v(PEAL_V_MW, PEAL_V_FACTOR_PLANTA,
                                           PEAL_V_AHORRO_USD_MWH, cc["wacc"])
    m18_base = ME.m18_opcion_real_peal_v(PEAL_V_I0_USDMM, s0_base, mkt["rf"], sigma_lme_anual)
    s0_min = ME.s0_ahorro_energia_peal_v(PEAL_V_MW, 0.40, PEAL_V_AHORRO_USD_MWH, cc["wacc"])
    s0_max = ME.s0_ahorro_energia_peal_v(PEAL_V_MW, 0.50, PEAL_V_AHORRO_USD_MWH, cc["wacc"])
    m18 = {
        **m18_base,
        "sigma_lme_anual": sigma_lme_anual,
        "s0_usdmm": s0_base, "s0_min_usdmm": s0_min, "s0_max_usdmm": s0_max,
        "capex_usdmm": PEAL_V_I0_USDMM, "mw": PEAL_V_MW,
        "factor_planta_base": PEAL_V_FACTOR_PLANTA, "ahorro_usd_mwh": PEAL_V_AHORRO_USD_MWH,
    }
...
def run(forzar_descarga=False) -> dict:
    ...
    res = {
        ...
        "m2b_validacion_econometrica": m2b_validacion_econometrica(panel, cc, pro, mkt, dn, est),
        "m3_macro": m3_macro(),
        "m4_estados": est,
        "m5_proyecciones": pro,
        "m6_costo_capital": cc,
