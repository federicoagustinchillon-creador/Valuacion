...
def build_full_word_document():
    ...
    cp.keywords = "Valuación, Aluar, ALUA.BA, DCF, WACC, Opciones Reales, Monte Carlo, GARCH, Cópula t-Student, FCE UNCuyo, Chillón"
    ...
    # Metadatos de fecha explícitos: evita que Word herede el created=2013-12-23
    # que trae el template en blanco por defecto de python-docx (artefacto
    # detectable como "documento generado programáticamente" al inspeccionar
    ...
    r_r1 = p_tr.add_run("Equity Research\nAnálisis Independiente\nFecha de Valuación: 23 de julio de 2026")
    ...
    add_rich_runs(p_aviso, "**Aviso legal:** Documento de análisis financiero independiente. Para el descargo de responsabilidad legal y certificaciones (Ley N.º 26.831), consulte la página 2. Datos de mercado (LME, EMBI+, CCL) al 23 de julio de 2026; no reflejan movimientos posteriores a esa fecha.", font_size=Pt(7.0), color=COLOR_GRAY, italic_all=True)
    ...
    add_rich_runs(p_lt1, "Valuación fundamental de **Aluar Aluminio Argentino S.A.I.C. (ALUA.BA)** con dictamen de **COMPRAR** y precio objetivo fundamental base de **ARS 1.236,00** por acción (+25,8% de retorno esperado frente a una cotización de mercado de ARS 982,50). El modelo integra asimismo el valor de la opción real de expansión eólica PEAL V (+ARS 119,10), alcanzando un Target Integrado de ARS 1.355,10 (+37,9%). La cotización actual descuenta un crecimiento terminal implícito deprimido (g* = 0,69%), subestimando la ventaja en costos por energía propia y el desapalancamiento financiero proyectado.", font_size=Pt(8.2), color=COLOR_DARK)
    ...
    add_rich_runs(p_bx0, "Dictamen del Modelo Fundamental", font_size=Pt(7.5), color=COLOR_GRAY)
    ...
    add_rich_runs(p_bx2, "Target Fundamental Base: **ARS 1.236,00** (USD 0,78)", font_size=Pt(8.0), color=COLOR_NAVY, bold_all=True)
    
    p_bx3 = box_cell.add_paragraph()
    p_bx3.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_bx3.paragraph_format.space_after = Pt(0)
    add_rich_runs(p_bx3, "Cotización de Mercado: ARS 982,50 | **Retorno: +25,8%**", font_size=Pt(7.5), color=COLOR_DARK)
    
    p_bx4 = box_cell.add_paragraph()
    p_bx4.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_bx4.paragraph_format.space_after = Pt(0)
    add_rich_runs(p_bx4, "Escenario Expansión (+ Opción Real PEAL V):", font_size=Pt(6.8), color=COLOR_GRAY)
    
    p_bx5 = box_cell.add_paragraph()
    p_bx5.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_bx5.paragraph_format.space_after = Pt(2)
    add_rich_runs(p_bx5, "Target Integrado: **ARS 1.355,10** (USD 0,86 | +37,9%)", font_size=Pt(7.5), color=COLOR_BLUE)
            ...
            if r'\end{figure}' in line or r'\end{figure*}' in line:
                ...
                if len(mini_matches) >= 2:
                    if r'\includegraphics' in mini_matches[0] and r'\includegraphics' in mini_matches[1]:
                        ...
                        cap_text = clean_full_latex_text(m_c.group(1)) if m_c else "Figura 31: Ajuste de Cópula de Clayton (dependencia de cola inferior); el ajuste por AIC favorece finalmente a la t-Student."
                        sobol_rows = [
