...
def plot_figura_03(res, stat):
    ...
    # Sin este margen explícito, el auto-escalado dejaba la etiqueta "-9,9%" (offset -1.8 respecto
    # a la barra) pegada al borde inferior del eje, superpuesta con el tick "2020".
    ...
    for i, v in enumerate(infl):
        # bbox blanco detrás del label: en los tramos de pendiente pronunciada (ej. 2024->2025,
        # 120%->32%) la propia línea roja pasaba por detrás de la etiqueta y cortaba el primer
        # dígito, volviendo "32%" ilegible como "22%".
...
def plot_figura_04(res, stat):
    ...
    pico = max(embi)
    caida_pct = (embi[-1] - pico) / pico * 100  # variación real desde el pico, no tipeada

    fig, ax = scaffold(
        f"Compresión del Riesgo País (EMBI+ Argentina): {caida_pct:.0f}% desde el Pico",
        "Trayectoria descendente del spread soberano tras la estabilización fiscal",
        "pb"
    )
    x = np.arange(len(dates))
    # Rojo/riesgo, no azul/neutro: coherente con el resto del informe, donde el color de riesgo
    # soberano (EMBI+, stress test, VaR) es consistentemente C["risk"], nunca C["blue"].
...
def plot_figura_13(res, stat):
    ...
    steps = ["1. OLS Bruto", "2. Ajuste Blume", "3. Desapalancado", "4. Hamada Reapalancado"]
...
def plot_figura_15(res, stat):
    ...
    Reescrita: la versión anterior (a) no era un waterfall real -- graficaba cada paso como una
    barra absoluta desde cero en vez de barras flotantes acumulativas, y (b) mezclaba en el mismo
    eje Y montos en USD MM (hasta ~2.640) con el Target en ARS/acción (~1.236), una unidad
    completamente distinta -- los dejaba a escala comparable por casualidad, no por diseño, lo
    cual es un error de visualización, no solo de estilo. Ahora: barras flotantes reales
    (bottom=) para el bloque en USD MM, y el Target ARS se muestra como anotación de flecha
    separada (como en el informe de referencia), no como una barra más en el mismo eje."""
    m7 = res["m7_dcf"]
    van_5y = m7["van_5y"]
    ...
    ev_pct_tv = vp_tv / ev * 100  # peso real del valor terminal sobre el EV, no tipeado

    fig, ax = scaffold(
        f"PV del Valor Terminal representa {ev_pct_tv:.1f}% del EV",
        "Puente de Valuación: del FCFF descontado al precio objetivo · USD MM",
        "USD MM"
    )
    steps = ["PV FCFF\nExplícitos", "+ PV Valor\nTerminal", "= EV", "− Deuda\nNeta", "= Equity", "= Target\nARS"]
    # barras flotantes reales: cada paso arranca donde terminó el anterior
    bottoms = [0, van_5y, 0, equity, 0, 0]
    heights = [van_5y, vp_tv, ev, deuda, equity, 0]  # el último paso (Target) no es una barra
    colors = [C["navy"], C["blue_lt"], C["navy"], C["risk"], C["value"], C["aluar"]]
    for i in range(5):
    ...
    # Barra de altura 0 solo para registrar la 6ta categoría en el eje categórico -- si no se
    # llama a ax.bar() para "= Target\nARS", matplotlib nunca le asigna una posición X y el
    # annotate() de más abajo termina apuntando a un x=5 que no existe en el eje (bug real:
    # la flecha del Target no aparecía en el render).
    ax.bar(steps[5], 0, color="none")
    ax.set_ylim(0, ev * 1.25)
    # Target ARS: unidad distinta (ARS/acción, no USD MM) -- se muestra como flecha + etiqueta,
    # no como una barra más en el mismo eje (evita mezclar escalas incompatibles).
...
def plot_figura_16(res, stat):
    ...
    ax.plot(x, dso, marker="s", color=C["blue"], lw=1.8, label="DSO (Días Cuentas por Cobrar)")
...
def plot_figura_19(res, stat):
    ...
    # RdYlGn (diverging) en vez de YlGnBu (secuencial azul): en una tabla de sensibilidad de
    # precio objetivo, rojo=peor/más bajo y verde=mejor/más alto es la convención institucional
    # estándar (igual que el resto del informe usa rojo=riesgo, verde=valor en todos lados);
    # un secuencial azul no comunica esa lectura de "peor a mejor".
    ...
    for i in range(len(g_rows)):
        for j in range(len(wacc_cols)):
            val = matrix[i, j]
            # RdYlGn es oscuro en AMBOS extremos (rojo bajo, verde alto) y claro en el medio
            # (amarillo) -- un único umbral (antes "> 1500") daba texto negro sobre rojo oscuro,
            # ilegible en la esquina de mayor riesgo de la matriz.
...
def plot_figura_24(res, stat):
    ...
    fig, ax = scaffold(
        ...
        f"Aísla la señal del ruido diario: el nivel reciente ({beta_actual:.3f}β) diverge del promedio histórico ({m6['beta_apalancado']:.3f}β)",
        "Beta (β)"
    )
    ax.plot(kb["date"], kb["beta_kalman"], color=C["navy"], lw=1.2, label="Beta Dinámico (Kalman Empírico)")
    ax.axhline(m6["beta_apalancado"], color=C["aluar"], linestyle="--", lw=1.5,
               label=f"Beta Hamada ({m6['beta_apalancado']:.3f}β)")
    ax.axhline(m6["beta_ols"], color=C["muted"], linestyle=":", lw=1.3,
...
def plot_figura_25(res, stat):
    ...
    # El eje ya está en unidades de % (x*100 arriba) pero sin el formateador quedaba con números
    # crudos (0, 5, 10...) sin el símbolo "%" -- inconsistente con el resto del informe.
...
def plot_figura_27(res, stat, muestra):
    ...
    # Antes solo mostraba la curva KDE suavizada, sin las barras del histograma real debajo (que sí
    # tiene el informe de referencia) ni la línea de Spot de mercado para comparar contra el Target.
    ax.hist(muestra, bins=50, range=(lo, hi), density=True, color=C["grid"], edgecolor="white", alpha=0.9, zorder=1)
    ax.plot(x, density, color=C["navy"], lw=2.2, zorder=3, label="Densidad Suavizada KDE")
    ax.fill_between(x, density, color=C["blue_lt"], alpha=0.15, zorder=2)
...
def plot_figura_30(res, stat):
    ...
    # Suavizado de ventana móvil (21 ruedas ~ 1 mes bursátil) real sobre la serie diaria real -- no
    # reemplaza el dato, solo lo hace legible: la serie diaria cruda (usada tal cual antes) tiene
    # picos de hasta 3.0beta que comprimen visualmente todo el resto de la trayectoria contra el
    # eje X, y no traía ni banda de confianza ni la referencia de Beta Hamada que sí tiene el
    # informe de referencia.
    # np.asarray() primero: beta_dinamico ya es una pd.Series (indexada 0..N-1, heredada de
    # sigma_alua_t=garch_fit.conditional_volatility). Pasarla directo a pd.Series(..., index=dates)
    # dispara el alineado automático de índices de pandas contra ese índice 0..N-1 en vez de contra
    # "dates" -- como los índices no coinciden, el resultado salía casi todo NaN (el gráfico entero
    # quedaba vacío, con el eje X colapsado a un solo timestamp repetido).
    ...
    # Hito real: pico de beta dinámico dentro de 2020 (no una fecha tipeada), consistente con el
    # shock de volatilidad de mercado documentado en el resto del informe (COVID-19).
    ...
    if mask_2020.any():
        ...
        ax.annotate(f"Shock COVID-2020\nPico: {valor_pico:.2f}β", xy=(fecha_pico, valor_pico),
                    ...
                    bbox=dict(boxstyle="round,pad=0.35", facecolor="#FFF9DB", edgecolor=C["gold"], lw=1.1))
    ax.set_xlabel("Año", fontsize=SZ["axis"])
    ax.legend(frameon=False, fontsize=SZ["legend"], loc="lower left")
...
def plot_figura_31(res, stat, muestra):
    ...
    # Recorte al percentil 0.5-99.5 de la MUESTRA COMPLETA (no de la submuestra de 1.000 puntos
    # graficados): mismo bug de cola pesada que en Fig.20/27 -- sin esto, un puñado de outliers de
    # la cola larga Student-t estiraban el eje X hasta ~2.500 dejando la nube de puntos real
    # comprimida en el 30% izquierdo del gráfico.
    lo_x, hi_x = np.percentile(muestra, [0.5, 99.5])
